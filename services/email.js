    ...
    <div style="font-family:Inter,Arial,sans-serif;background:#f8fafc;padding:32px;color:#0f172a;">
      <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e2e8f0;">
        <div style="background:#c8102e;color:#ffffff;padding:24px 28px;">
          <div style="font-size:12px;letter-spacing:.08em;text-transform:uppercase;opacity:.85;">Think Union Credit Bank</div>
...
async function sendAccountWelcomeEmail(user) {
  ...
  const title = 'Welcome to Think Union Credit Bank';
  const body = `
  ...
  return sendEmail({
    to: user.email,
    subject: 'Welcome to Think Union Credit Bank',
    html: buildEmailFrame(title, body),
    text: `Welcome ${user.first_name || ''}. Your Think Union Credit Bank online banking profile has been created successfully.`
  });
}
...
async function sendLoginWelcomeEmail(user, meta = {}) {
  ...
  const title = 'Welcome back to Think Union Credit Bank';
  const body = `
    <p style="margin:0 0 16px;font-size:15px;">Hello ${escapeHtml(user.first_name || 'there')},</p>
    <p style="margin:0 0 16px;font-size:15px;line-height:1.7;">
  ...
  return sendEmail({
    to: user.email,
    subject: 'Successful login to your Think Union Credit Bank account',
    html: buildEmailFrame(title, body),
...
async function sendPasswordChangedEmail(user) {
  ...
  return sendEmail({
    ...
    text: 'Your Think Union Credit Bank password has been changed successfully.'
  });
}
