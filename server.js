...
const sessionSecret = process.env.SESSION_SECRET || 'think-union-credit-bank-super-secret-key-2026';
...
(function runFirestoreStartupCheck() {
  ...
  if (!enabled) {
    ...
    if (diag.runtime === 'vercel') {
      ...
      console.log('     • Optional: FIREBASE_PROJECT_ID (e.g. thinkunioncb-4845b)');
      console.log('');
...
app.use((req, res, next) => {
  ...
  res.locals.jivoWidgetId = jivoWidgetId;
...
app.use((req, res) => {
  res.status(404).render('errors/404', {
    title: 'Page Not Found - Think Union Credit Bank',
    page: '404'
  });
});
...
app.use((err, req, res, next) => {
  ...
  res.status(500).render('errors/500', {
    title: 'Server Error - Think Union Credit Bank',
    page: '500',
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`Think Union Credit Bank server running on http://localhost:${PORT}`);
  });
}
